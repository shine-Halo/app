  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(item,index) in userAddressList" :key="item.id">
        ...
        <p @click="faultaddress(item,userAddressList)">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{confirminfo.fullAddress}}</span>
        收货人：<span>{{confirminfo.consignee}}</span>
        <span>{{confirminfo.phoneNum}}</span>
      </div>
    </div>
    <div class="sub clearFix">
      <!-- <router-link  to="/pay">提交订单</router-link>不能立即跳，所以改成a链接 -->
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      // 计算是哪一个收件人的详细信息，find是只要是碰到符合的就返回
      confirminfo() {
        return this.userAddressList.find(item => item.isDefault == 1 ) || {}
      }
    },
    methods: {
     faultaddress(currentitem,userAddressList) {
      userAddressList.forEach(item => {
        item.isDefault = 0
        currentitem.isDefault = 1
      })
     },
     async submitorder() {
      // console.log(this.$API);vc可以访问到vue原型上的属性和方法
      let {tradeNo} = this.tradeinfo
      let data = {
        "consignee": this.confirminfo.consignee,
        "consigneeTel": this.confirminfo.phoneNum,
        "deliveryAddress": this.confirminfo.fullAddress,
        "paymentWay": "ONLINE",
        "orderComment": this.message,//订单备注
        "orderDetailList": this.detailArrayList
     }
        let result =await this.$API.getorder(tradeNo,data)
        // console.log(result);
        if(result.code == 200) {
