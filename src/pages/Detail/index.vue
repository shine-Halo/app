          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item,index) in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0" :class="{active:itemcolor.isChecked == 1}" v-for="(itemcolor,index) in item.spuSaleAttrValueList" :key="itemcolor.id" @click="changeactive(itemcolor,item.spuSaleAttrValueList)">{{itemcolor.saleAttrValueName}}</dd>
              </dl>
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="thing">
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$store.dispatch('reggoodslist',this.$route.params.skuId)
    },
    ...
    methods: {
      changeactive(selected,checkround) {
        //一开始样式全部移除，只有单机的才有样式,排他思想
        checkround.forEach(item => {
      ...
      console.log(num);
      //如果输入数据是文字加数字或者文字就是NaN,是nan或者小于0的都不行
      if(isNaN(num)  || num < 0) {
        this.skuNum = 1
        console.log(9);
      }else {
    ...
    async addcar() {
      ...
      //3 失败：给用户进行提示
      try {
         await this.$store.dispatch('regdetail',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
