<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>确认密码:</label>
        <input type="password" placeholder="请确认你的密码" v-model="password1" name="password1" v-validate="{required:true,is:password }" :class="{invalid:errors.has('password1')}"/>
        <span class="error-msg">{{errors.first("password1")}}</span>
      </div>
      <div class="controls">
        <input type="checkbox" placeholder="请勾选" v-model="arg" name="arg" v-validate="{required:true,'agree':true }" :class="{invalid:errors.has('arg')}"/>
        <span class="error-msg">{{errors.first("arg")}}</span>
        <!-- <input name="m1" type="checkbox" :checked="arg"> -->
        <span>同意协议并注册《尚品汇用户协议》</span>
        <!-- <span class="error-msg">错误提示信息</span> -->
      </div>
      <div class="btn">
        <button @click="registerd">完成注册</button>
      </div>
    </div>
...
<script>
  export default {
    ...
    methods: {
      async getcode(phone) {
        (this.phone)&&await this.$store.dispatch('regregister',phone)
