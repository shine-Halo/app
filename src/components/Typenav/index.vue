<template>
   <div class="type-nav">
    <div class="container">
        <!--通过事件委派实现-->
    <div @mouseleave="outindex">
        <h2 class="all">全部商品分类</h2>
        <!-- 三级联动 -->
    <div class="sort">
        <!-- 事件委派 -->
    <div class="all-sort-list2" @click="gosearch">
    <div class="item" v-for="(c1,index) in result" :key="c1.categoryId" :class="{cur: currentindex == index}" >
        <h3 @mouseenter="enterindex(index)">
            <a :data-category="c1.categoryName" :data-category1id="c1.categoryId">{{c1.categoryName}}</a>
        </h3>
        ...
        <div class="subitem"  v-for="(c2,index)  in  c1.categoryChild"  :key="c2.categoryId" >
            <dl class="fore">
                <dt>
                    <a :data-category="c2.categoryName" :data-category2id="c2.categoryId">{{c2.categoryName}}</a>
                </dt>
                <dd >
                    <em v-for="(c3,index)  in c2.categoryChild" :key="c3.categoryId">
                        <a :data-category="c3.categoryName" :data-category3id="c3.categoryId">{{c3.categoryName}}</a>
                    </em>                                  
                </dd>
            </dl>
            </div>
    ...
    methods:{
      ...
      enterindex:_.throttle(function(index) {
        ...
        gosearch() {
            ...
            const {category,category1id,category2id,category3id} = element.dataset
